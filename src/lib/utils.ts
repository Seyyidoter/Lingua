...
export const fuzzyEq = (val?: string, expected?: string) => {
  const v = (val ?? "").trim().toLowerCase();
  const e = (expected ?? "").trim().toLowerCase();

  if (!v) return false;

  const rawCandidates = e.split(/[;,\/]+/).map((s) => s.trim());
  ...
  rawCandidates.forEach((cand) => {
    ...
    if (clean && clean !== cand) {
      expandedCandidates.push(clean);
    }
  });

  // 3. Kontrol et
  return expandedCandidates.some((cand) => {
    // Tam eşleşme veya 1 harf hatası (typo)
    return cand === v || levenshtein(cand, v) <= 1;
  });
};
